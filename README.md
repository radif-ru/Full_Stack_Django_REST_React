...
> `python manage.py add_users` - скрипт заполнения таблиц БД дефолтным админом и пользователями. Так же админ добавляется в группу разрешений Администратор, остальные дефолтные пользователи попадают в рандомные группы разрешений из БД. В группу Владелец проекта будут попадать те, кто будут создавать свои проекты

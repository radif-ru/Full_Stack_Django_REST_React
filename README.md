...
>`/api/users/<int:pk>/` - GET, PUT, PATCH, HEAD, OPTIONS - в зависимости от метода запроса - вывод данных пользователя по `id` в БД, редактирование и т.д. Удаление запрещено!

> `api/users/?login=<логин>` - GET, HEAD, OPTIONS - вывод данных пользователей отфильтрованных по логину (в БД username), вместо `<логин>` подставляем уникальный логин пользователя. Настроено вручную стандартными методами

> `api/users/?first_name=<имя>&middle_name=<фамилия>&last_name=<отчество>` - GET, HEAD, OPTIONS - вывод данных пользователей, отфильтрованных по имени, фамилии, отчеству вместо `<имя>` подставляем имя, вместо `<фамилия>` фамилию, вместо `<отчество>` отчество пользователей. Некоторые данные можно опускать или комбинировать, в том числе с другими методами фильтрации, как например по логину выше. Настроено с помощью `django-filter`

>`/api/users/<int:pk>/login/` - GET, HEAD, OPTIONS - вывод уникального логина пользователя по id в БД

>`/api/users/<int:pk>/fio/` - GET, HEAD, OPTIONS - вывод фамилии, имени, отчества пользователя по id в БД
...
>`/api/projects/?page=<int>` - GET, POST, HEAD, OPTIONS - вместо `<int>` указывается нужная страница с заметками. В ответ кроме проектов приходят ссылки на смещение относительно страницы и другие данные
...
>`/api/todos/?page=<int>` - GET, POST, HEAD, OPTIONS - вместо `<int>` указывается нужная страница с заметками. В ответ кроме заметок приходят ссылки на смещение относительно страницы и другие данные
